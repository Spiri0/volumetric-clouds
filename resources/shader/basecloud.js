export const basecloudFS = `
	...
	float hash(int n){return fract(sin(float(n) + 1.951) * 43758.5453123);}
	...
	vec4 mod289(vec4 x){return x - floor(x * vec4(1.0) / vec4(289.0)) * vec4(289.0);}
	...
	{
	//	vec3 coord = pixel;//vec3(float(pixel.x) / 128.0, float(pixel.y) / 128.0, float(pixel.z) / 128.0);
		...
		{
			float cellCount = 4.0;
			float worleyNoise0 = (1.0 - worleyNoise3D(coord, cellCount * 2.0));
			float worleyNoise1 = (1.0 - worleyNoise3D(coord, cellCount * 8.0));
			float worleyNoise2 = (1.0 - worleyNoise3D(coord, cellCount * 14.0));
			float worleyNoise3 = (1.0 - worleyNoise3D(coord, cellCount * 20.0));
			float worleyNoise4 = (1.0 - worleyNoise3D(coord, cellCount * 26.0));
			float worleyNoise5 = (1.0 - worleyNoise3D(coord, cellCount * 32.0));
	...
	void main() { 
		...
		vec4 result = stackable3DNoise(pixel);
